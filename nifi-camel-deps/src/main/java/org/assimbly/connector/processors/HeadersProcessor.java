    ...
    if (endpointIdObject != null) {
      ...
      for (Map.Entry<String, String> entry : headers.entrySet()) {
        ...
        if (key.startsWith("header." + endpointId + ".constant")) {
          ...
          in.setHeader(
              StringUtils.substringAfterLast(key, "simple."),
              simple.createExpression(entry.getValue()).evaluate(exchange, String.class));
        } else if (key.startsWith("header." + endpointId + ".xpath")) {
          XPathFactory fac = new net.sf.saxon.xpath.XPathFactoryImpl();
          String xpathResult =
              XPathBuilder.xpath(entry.getValue()).factory(fac).evaluate(exchange, String.class);
