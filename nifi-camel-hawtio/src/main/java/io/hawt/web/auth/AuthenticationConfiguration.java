...
public class AuthenticationConfiguration {
  ...
  private static final transient Logger LOG =
      LoggerFactory.getLogger(AuthenticationConfiguration.class);
  ...
  public static final String AUTHENTICATION_CONTAINER_DISCOVERY_CLASSES =
      "authenticationContainerDiscoveryClasses";
  ...
  public AuthenticationConfiguration(ServletContext servletContext) {
    ConfigManager config =
        (ConfigManager) servletContext.getAttribute(ConfigManager.CONFIG_MANAGER);
    ...
    if (config != null) {
      ...
      this.enabled = config.getBoolean(AUTHENTICATION_ENABLED, false);
      ...
      authDiscoveryClasses =
          config.get(AUTHENTICATION_CONTAINER_DISCOVERY_CLASSES, authDiscoveryClasses);
    }
    ...
    if (this.enabled) {
      ...
      for (AuthenticationContainerDiscovery discovery : discoveries) {
        if (discovery.canAuthenticate(this)) {
          LOG.info(
              "Discovered container {} to use with hawtio authentication filter",
              discovery.getContainerName());
          break;
        }
      }
    }
    ...
    if (this.enabled) {
      LOG.info(
          "Starting hawtio authentication filter, JAAS realm: \"{}\" authorized role(s): \"{}\""
              + " role principal classes: \"{}\"",
          this.realm,
          this.role,
          this.rolePrincipalClasses);
    } else {
  ...
  public static AuthenticationConfiguration getConfiguration(ServletContext servletContext) {
    AuthenticationConfiguration authConfig =
        (AuthenticationConfiguration) servletContext.getAttribute(AUTHENTICATION_CONFIGURATION);
    if (authConfig == null) {
  ...
  private static List<AuthenticationContainerDiscovery> getDiscoveries(
      String authDiscoveryClasses) {
    ...
    for (String discoveryClass : discoveryClasses) {
      try {
        ...
        Class<? extends AuthenticationContainerDiscovery> clazz =
            (Class<? extends AuthenticationContainerDiscovery>)
                AuthenticationConfiguration.class.getClassLoader().loadClass(discoveryClass.trim());
